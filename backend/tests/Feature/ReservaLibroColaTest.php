...
class ReservaLibroColaTest extends TestCase
{
    ...
    public function test_no_puede_unirse_dos_veces_a_la_cola_del_mismo_libro(): void
    {
        ...
        $this->assertSame(1, ReservaLibro::where('libro_id', $libro->id)->where('usuario_id', $usuario->id)->count());
    }
    ...
    public function test_index_incluye_la_posicion_en_la_cola(): void
    {
        ...
        $this->postJson('/api/reservas-libro', ['usuario_id' => $primero->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201);
        $this->postJson('/api/reservas-libro', ['usuario_id' => $segundo->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201);
    ...
    public function test_devolver_prestamo_promueve_al_primero_de_la_cola_en_vez_de_liberar_el_libro(): void
    {
        Sanctum::actingAs(Staff::factory()->create());
        $libro = Libro::factory()->create(['disponible' => false]);
        ...
        $this->postJson('/api/reservas-libro', ['usuario_id' => $primeroEnCola->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201);
        $this->postJson('/api/reservas-libro', ['usuario_id' => $segundoEnCola->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201);

        $this->patchJson("/api/prestamos/{$prestamo->id}/devolver")->assertStatus(200);

        // El libro sigue "no disponible" — quedó apartado para el primero de la cola.
        $this->assertDatabaseHas('libros', ['id' => $libro->id, 'disponible' => false]);

        $reservaPrimero = ReservaLibro::where('libro_id', $libro->id)->where('usuario_id', $primeroEnCola->id)->first();
        $this->assertSame('pendiente', $reservaPrimero->estado);
        $this->assertNotNull($reservaPrimero->fecha_retiro);

        $reservaSegundo = ReservaLibro::where('libro_id', $libro->id)->where('usuario_id', $segundoEnCola->id)->first();
        $this->assertSame('en_cola', $reservaSegundo->estado);
    }

    public function test_devolver_prestamo_sin_nadie_en_cola_deja_el_libro_disponible(): void
    {
        Sanctum::actingAs(Staff::factory()->create());
        $libro = Libro::factory()->create(['disponible' => false]);
        $prestamo = Prestamo::factory()->create([
    ...
    public function test_cancelar_reserva_pendiente_promueve_al_primero_de_la_cola(): void
    {
        ...
        $this->postJson('/api/reservas-libro', ['usuario_id' => $enCola->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201);
    ...
    public function test_cancelar_reserva_en_cola_no_afecta_disponibilidad_ni_promueve_a_nadie(): void
    {
        ...
        $segundo = Usuario::factory()->create();
        $this->postJson('/api/reservas-libro', ['usuario_id' => $primero->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201);
        $reservaSegundo = $this->postJson('/api/reservas-libro', ['usuario_id' => $segundo->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201)->json();

        $this->patchJson("/api/reservas-libro/{$reservaSegundo['id']}/cancelar")->assertStatus(200);

        // El libro sigue igual (ocupado por el préstamo/reserva original, no por la cola).
        $this->assertDatabaseHas('libros', ['id' => $libro->id, 'disponible' => false]);
    ...
    public function test_orden_fifo_de_la_cola_se_respeta_con_tres_personas(): void
    {
        ...
        foreach ($usuarios as $usuario) {
            $this->postJson('/api/reservas-libro', ['usuario_id' => $usuario->id, 'codigo_barras' => $libro->codigo_barras])->assertStatus(201);
        ...
        foreach ($usuarios as $usuario) {
            $reserva = ReservaLibro::where('libro_id', $libro->id)->where('usuario_id', $usuario->id)->first();
        ...
        // Nota: el libro ya estaba "ocupado" por este préstamo simulado (no por otra
        // reserva), pero lo relevante acá es que liberarLibro() (llamado por devolver())
        // promueva siempre al más antiguo de la cola.
        $this->patchJson("/api/prestamos/{$prestamo->id}/devolver")->assertStatus(200);

        $primero = ReservaLibro::where('libro_id', $libro->id)->where('usuario_id', $usuarios[0]->id)->first();
        $this->assertSame('pendiente', $primero->estado);

        foreach ([1, 2] as $idx) {
            $resto = ReservaLibro::where('libro_id', $libro->id)->where('usuario_id', $usuarios[$idx]->id)->first();
